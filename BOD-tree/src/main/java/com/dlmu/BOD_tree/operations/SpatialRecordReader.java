...
import com.dlmu.BOD_tree.core.Partition;
...
/**
 * A base class to read shapes from files. It reads either single shapes, list
 * of shapes, or R-trees. It automatically detects the format of the underlying
 * block and parses it accordingly.
 * 
 * The class implement the RecordReader interface allowing it to be used in
 * MapReduce programs with an appropriate InputFormat. The key is always a
 * {@link Rectangle} that indicates the MBR of the corresponding partition. In
 * case of a non-indexed file, the key is an invalid rectangle. See
...
public abstract class SpatialRecordReader<K, V> implements RecordReader<K, V> {
	...
	/**
	 * The input stream that reads directly from the input file. If the file is not
	 * compressed, this stream is the same as the in. Otherwise, this is the raw
	 * (compressed) input stream. This stream is used to calculate the progress of
	 * the input file.
	 */
	...
	public SpatialRecordReader(Configuration job, long s, long l, Path p) throws IOException {
		...
		if (isCompressedInput()) {
			...
			if (codec instanceof SplittableCompressionCodec) {
				final SplitCompressionInputStream cIn = ((SplittableCompressionCodec) codec).createInputStream(directIn,
	...
	/**
	 * Returns the current position of the file being parsed. This is equal to the
	 * number of bytes consumed from disk regardless of whether the file is
	 * compressed or not. This function is used to report the progress.
	 * 
	 * @return
	 * @throws IOException
	 */
	...
	/**
	 * Returns the current position in the data file. If the file is not compressed,
	 * this is equal to the value returned by {@link #getFilePosition()}. However,
	 * if the file is compressed, this value indicates the position in the
	 * decompressed stream.
	 */
	...
	/**
	 * Initializes the reader to read from the input stream or file. First, it
	 * initializes the MBR of the partition being read if the file is globally
	 * indexed. It also detects whether the file is R-tree indexed or not which
	 * allows it to skip the R-tree if not needed to be read.
	 * 
	 * @throws IOException
	 */
	protected boolean initializeReader() throws IOException {
		...
		if (path != null) {
			GlobalIndex<Partition> globalIndex = SpatialSite.getGlobalIndex(fs, path.getParent());
			if (globalIndex != null) {
	...
	/**
	 * Reads the next line from input and return true if a line was read. If no more
	 * lines are available in this split, a false is returned.
	 * 
	 * @param value
	 * @return
	 * @throws IOException
	...
	/**
	 * Reads next shape from input and returns true. If no more shapes are left in
	 * the split, a false is returned. This function first reads a line by calling
	 * the method {@link #nextLine(Text)} then parses the returned line by calling
	 * {@link Shape#fromText(Text)} on that line. If no stock shape is set, a
	...
	/**
	 * Reads all shapes left in the current block in one shot. This function runs a
	 * loop where it keeps reading shapes by calling the method
	 * {@link #nextShape(Shape)} until one of the following conditions happen. 1.
	 * The whole file is read. No more records to read. 2. Number of parsed records
	 * reaches the threshold defined by the configuration parameter
	 * spatialHadoop.mapred.MaxShapesPerRead. To disable this check, set the
	 * configuration parameter to -1 3. Total size of parsed data from file reaches
	 * the threshold defined by the configuration parameter
	 * spatialHadoop.mapred.MaxBytesPerRead. To disable this check, set the
	...
	/**
	 * Reads the next RTree from file. The file must be part of an R-tree index. If
	 * the file is not locally indexed using an R-tree, a runtime exception is
	 * thrown. If the file is locally indexed using an R-tree, the R-tree is
	 * consumed from the file and parsed by calling
	 ...
	 * @throws IOException
	 */
	// protected boolean nextRTree(RTree<? extends Shape1D> rtree) throws
	// IOException {}
}
